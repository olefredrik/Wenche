"""
Dokumentgenerering for forhåndsvisning/nedlasting i self-hosted Wenche (porten fra
Dokumenter-fanen i `wenche/ui.py`). Genererer lokalt, sender ingenting. Hvert svar er en liste
filer med base64-kodet innhold som SPA-en kan tilby som nedlasting.
"""
...
def _bygg_noter(config: dict) -> Noter:
    """Bygg Noter-objektet fra config-dict (samme form som NiceGUI persisterte)."""
...
def dok_aksjonaer(config: dict[str, Any] = Body(...)) -> dict:
    config = miljo.med_test_org(config)
...
@router.post("/noter")
def dok_noter(config: dict[str, Any] = Body(...)) -> dict:
    config = miljo.med_test_org(config)
    regnskap = ar.les_config(config)
